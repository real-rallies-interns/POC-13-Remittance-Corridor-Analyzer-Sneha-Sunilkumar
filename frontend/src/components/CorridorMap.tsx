'use client'
/**
 * CorridorMap.tsx — v4.0
 * Curved arc corridors with heatmap colour coding.
 *
 * KEY FIX: Curved arcs using bezier midpoint offset.
 * UAE has 3 corridors (UAE-PK, UAE-IN) starting from same origin.
 * Without curves they overlap into one line — with curves each is visible.
 *
 * Heatmap colours by heat_intensity:
 *   ≥0.8 → #F87171 Red    (Very High volume)
 *   ≥0.6 → #FBBF24 Amber  (High volume)
 *   ≥0.4 → #38BDF8 Cyan   (Medium volume)
 *   <0.4 → #818CF8 Indigo (Low volume)
 */
...
// Generate curved bezier arc points between two coordinates
...
// Unique curve offset per corridor — separates overlapping arcs from same origin
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      corridors.forEach(c => {
        ...
        const tip = `
          <div style="min-width:200px;background:#0B1117;border:1px solid #1F2937;
        ...
        // Invisible tooltip trigger at arc midpoint
        ...
        const fromM = L.marker([c.from_lat, c.from_lng], {
          icon: L.divIcon({
            html: `<div style="width:${active?14:9}px;height:${active?14:9}px;
              ...
              border:2px solid #030712;cursor:pointer"></div>`,
        ...
        const toM = L.marker([c.to_lat, c.to_lng], {
          icon: L.divIcon({
            html: `<div style="width:${active?9:6}px;height:${active?9:6}px;
              border-radius:50%;background:${color};
              opacity:${active?1:0.65};border:1.5px solid #030712;cursor:pointer"></div>`,
